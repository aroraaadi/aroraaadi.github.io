/* Search-driven mean-variance optimization over the S&P 500 + held names.
   ...
   Frank-Wolfe (no matrix inverse). The optimisation maths below is unchanged;
   only the module wiring, colour handling and keyboard access were reworked. */
...
function renderChips() {
  const chipHost = document.getElementById("chips"); chipHost.innerHTML = "";
  for (const sym of state.selected) {
    ...
    chip.append(sw, s, x); chipHost.appendChild(chip);
  }
}
...
function renderTiles(o, st) {
  const tileHost = document.getElementById("tiles"); tileHost.innerHTML = "";
  ...
  for (const d of defs) {
    ...
    t.append(l, v); tileHost.appendChild(t);
  }
}
...
function renderFrontier(st, frontier, minVar, maxSharpe) {
  ...
  if (state.showCloud) {
    ...
    datasets.push({ type: "scatter", label: "Random", data: cloud, pointRadius: 2, pointHoverRadius: 2,
      backgroundColor: alpha("--ramp-350", 0.27), order: 5 });
  }
  if (!riskOnly) {
  ...
  describeCanvas("frontier-chart",
    `Efficient frontier for ${st.syms.length} assets: ${st.syms.join(", ")}.`);
  drawChart("frontier-chart", {
    data: { datasets },
    options: {
...
function renderHeatmap(st) {
  ...
  if (!host) return;   // correlation panel not present on this page
  const k = st.k, cov = st.cov, vol = st.vol;
  const corr = (i, j) => cov[i][j] / (vol[i] * vol[j]);
  host.className = "heatmap";
  host.style.gridTemplateColumns = `auto repeat(${k}, 30px)`;
  host.innerHTML = "";
  host.appendChild(cell("", "hm-corner"));                       // top-left corner
  for (let j = 0; j < k; j++) host.appendChild(cell(st.syms[j], "hm-collabel"));
  for (let i = 0; i < k; i++) {
    host.appendChild(cell(st.syms[i], "hm-rowlabel"));
    for (let j = 0; j < k; j++) {
      const c = corr(i, j);
